Feature: 투두를 관리한다.
  ...
  Scenario Outline: 필수 투두를 수정한다.
    ...
    When "jinwoo"가 찾은 회차의 필수 투두를 수정 내용 "<수정 투두 체크 여부>", "<수정 투두 내용>"으로 수정한다.
    When "jinwoo"가 이름이 "자바1"인 스터디의 1 회차를 찾는다.

    Then 수정된 내용 "<수정 투두 체크 여부>", "<수정 투두 내용>" 이 필수 투두에 반영된다.

    Examples:
      | 투두 내용  | 수정 투두 체크 여부 | 수정 투두 내용 |  |
      | 새로운 투두 | true        | 수정됨      |  |
  ...
  Scenario Outline: 개설자가 아니면 필수 투두를 수정할 수 없다.
    ...
    When "yujamint"가 찾은 회차의 필수 투두를 수정 내용 "<수정 투두 체크 여부>", "<수정 투두 내용>"으로 수정한다.
