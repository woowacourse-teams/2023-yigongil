...
class FinishedStudyViewHolder(
    ...
    fun bind(finishedStudy: FinishedStudy) {
        binding.ivFinishedStudyTier.imageView(TierImage.valueOf(finishedStudy.averageTier))
        binding.ivFinishedStudyTag.imageView(FinishedStudyStatus.imageOf(finishedStudy.isSucceed))
    ...
    private fun ImageView.imageView(image: Int) {
        Glide.with(context)
            .load(image)
            .into(this)
    }
